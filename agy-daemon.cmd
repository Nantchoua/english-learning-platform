...
>>"%RUNNER%" echo "%AGY%" --remote-control%NAMEARG% ^>^> "%LOG%" 2^>^&1
...
call :check_auth
if "%AUTHED%"=="1" (
    echo Notice: Already authenticated ^(%TOKEN%^); skipping login.
    exit /b 0
)
echo --------------------------------------------------------------
...
if "%AUTHED%"=="1" (
    ...
    echo Warning: No auth token found at %TOKEN%; the daemon may start unauthenticated. >&2
...
:usage
echo Usage: %SELF% [install ^| status ^| restart ^| uninstall] [options]
echo.
